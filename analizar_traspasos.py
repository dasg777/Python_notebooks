...
def analizar_traspasos():
    logging.debug("Inicio de la función analizar_traspasos.")
    ...
    try:
        bk = xw.Book(file_path)
        logging.info("Archivo de Excel cargado exitosamente.")
    except Exception as e:
        logging.error(f"Error al cargar el archivo de Excel: {e}")
        return

    # Selecciona la hoja de trabajo
    sht = bk.sheets['tablas']
    logging.debug("Hoja de trabajo 'tablas' seleccionada.")

    # Restablecer status
    sht.range("status").value = 'Procesando...'
    logging.debug("Estado actualizado a 'Procesando...'")
    ...
    try:
        ...
        # Valor para decidir a quien se la da prioridad como sucursal (ordenada segun ventas totales, de mayor a menor; revisar orden)
        orden_prioridad_df = table_to_df(sht.tables['prioridad_sucursal'])
        logging.info("Tablas de Excel convertidas a DataFrames.")
    except Exception as e:
        logging.error(f"Error al convertir tablas de Excel a DataFrames: {e}")
        return
    ...
    try:
        deficit_df = deficit_df.merge(orden_prioridad_df, on='sucursal')
        deficit_df = deficit_df.sort_values(by='orden')
        logging.debug("Déficits ordenados por prioridad de sucursal.")
    except Exception as e:
        logging.error(f"Error al ordenar los déficits: {e}")
        return

    # Variables desde Excel
    try:
        kms_maximos_distancia = sht.range("kmsMaximosDistancia").value
        max_exced_permitido_AB = float(sht.range("max_exced_permitido_AB").value)
        max_inv_disp_C = float(sht.range("max_inv_disp_C").value)
        w_P = float(sht.range("w_P").value)
        w_C = float(sht.range("w_C").value)
        w_I = float(sht.range("w_I").value)
        w_E = float(sht.range("w_E").value)
        logging.debug("Variables cargadas desde Excel.")
    except Exception as e:
        logging.error(f"Error al cargar variables desde Excel: {e}")
        return

    # Filtrar matriz de traspasos, según kms máximos y elegibilidad
    try:
        prioridades_df = prioridades_df[(prioridades_df['prioridad'] <= kms_maximos_distancia) & (prioridades_df['elegible'] == 1)]
        logging.debug("Matriz de traspasos filtrada según kms máximos y elegibilidad.")
    except Exception as e:
        logging.error(f"Error al filtrar matriz de traspasos: {e}")
        return
    ...
    logging.debug("Normalización de valores completada.")
    ...
    def calcular_puntuacion(s_origen, s_destino, producto):
        try:
            ...
            # Normalización
            P_norm = (prioridad_max - prioridad + 1) / prioridad_max
            C_norm = clasificacion_val / clas_max
            I_norm = inventario_ajustado / inventario_max
            E_norm = excedente / excedente_max
            ...
            logging.error(f"Error en calcular_puntuacion: {e}")
    ...
    def satisfacer_deficit(deficit_row):
        try:
            ...
            logging.error(f"Error en satisfacer_deficit: {e}")
            return [], deficit_row['unidades_necesarias']
    ...
    for idx in range(total_deficits):
        ...
        progress = f'{idx + 1} de {total_deficits}'
        sht.range("status").value = progress
        logging.debug(f"Procesando déficit {idx + 1} de {total_deficits}.")
    ...
    traspasos_df = pd.DataFrame(todos_traspasos, columns=['sucursal_origen', 'sucursal_destino', 'producto', 'unidades_traspaso', 'costo_unitario'])
    traspasos_df = traspasos_df[traspasos_df['unidades_traspaso'] != 0]
    logging.info(f"Traspasos realizados: {traspasos_df.shape[0]} filas.")
    ...
    logging.info("DataFrames de traspasos y déficits residuales creados y ordenados.")
    ...
    def df_to_table(sheet, table_name, df):
        try:
            ...
            if not df.empty:
                start_cell = header_range.offset(1, 0)
                start_cell.resize(df.shape[0], df.shape[1]).value = df.values.tolist()
            logging.debug(f"Tabla {table_name} actualizada con {df.shape[0]} filas.")
        except Exception as e:
            logging.error(f"Error al actualizar la tabla {table_name}: {e}")
